...
function largestRectangleArea(heights: number[]): number {
    var stack = []
    var nextSmall = new Array(heights.length).fill(heights.length)
    var prevSmall = new Array(heights.length).fill(-1)
    ...
    var maxArea = 0
    for (var i = 0; i < heights.length; i++) {
        var currentArea = heights[i] * (nextSmall[i] - prevSmall[i] - 1)
